...
function renderGameStats(){
 ...
 try{
   ...
   const segment=(x,i)=>{const img=gameImage(game(x.id));const share=Number(x.share)||0;const count=Number(x.count)||0;const imageStyle=img?`background-image:url('images/${encodeURIComponent(img).replace(/%2F/g,'/')}');`:'';return `<button type="button" class="game-share-segment game-share-${i+1}" data-stat-filter="${escape(x.id)}" style="--segment-width:${share}%;${imageStyle}" title="${escape(x.name)} · ${count} directos · ${share.toFixed(1)}%"><span class="game-share-overlay" aria-hidden="true"></span></button>`};

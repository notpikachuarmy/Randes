...
function renderGameStats(){
 ...
 try{
   ...
   const segment=(x,i)=>{const img=gameImage(game(x.id));const share=Number(x.share)||0;const count=Number(x.count)||0;return `<button type="button" class="game-share-segment game-share-${i+1}" data-stat-filter="${escape(x.id)}" style="--segment-width:${share}%;" title="${escape(x.name)} · ${count} directos · ${share.toFixed(1)}%">${img?`<img src="images/${escape(img)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'">`:''}<span class="game-share-overlay" aria-hidden="true"></span></button>`};

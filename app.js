...
function renderGameStats(){
 ...
 try{
   ...
   const segment=(x,i)=>{const img=gameImage(game(x.id));const share=Number(x.share)||0;const count=Number(x.count)||0;const image=img?`<img src="images/${escape(img)}" alt="" draggable="false" loading="eager" decoding="async" onerror="this.style.display='none'">`:'';return `<button type="button" class="game-share-segment game-share-${i+1}" data-stat-filter="${escape(x.id)}" style="--segment-width:${share}%;" title="${escape(x.name)} · ${count} directos · ${share.toFixed(1)}%">${image}<span class="game-share-overlay" aria-hidden="true"></span></button>`};
   const othersSegment=others?`<button type="button" class="game-share-segment game-share-others" data-stat-filter="others" style="--segment-width:${others/total*100}%" title="Otros · ${others} directos · ${Number(total ? others/total*100 : 0).toFixed(1)}%"><span class="game-share-overlay" aria-hidden="true"></span></button>`:'';

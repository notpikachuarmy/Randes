...
function renderGameStats(){
 ...
 try{
   ...
   const segment=(x,i)=>{const img=gameImage(game(x.id));const share=Number(x.share)||0;const count=Number(x.count)||0;const image=img?`<img src="images/${escape(img)}" alt="" draggable="false" loading="eager" decoding="async" onerror="this.remove()">`:'';return `<button type="button" class="game-share-segment game-share-${i+1}" data-stat-filter="${escape(x.id)}" title="${escape(x.name)} · ${count} directos · ${share.toFixed(1)}%">${image}<span class="game-share-overlay" aria-hidden="true"></span></button>`};
   const othersSegment=others?`<button type="button" class="game-share-segment game-share-others" data-stat-filter="others" title="Otros · ${others} directos · ${Number(total ? others/total*100 : 0).toFixed(1)}%"><span class="game-share-overlay" aria-hidden="true"></span></button>`:'';
   host.innerHTML=`<div class="game-stats-intro"><div><p class="eyebrow">STREAM DISTRIBUTION</p><h2>Juegos más jugados</h2><p>Distribución de todos los directos completados. Los seis juegos principales aparecen individualmente y el resto se agrupa en «Otros».</p></div><div class="game-stats-total"><strong>${total}</strong><span>directos contabilizados</span></div></div><div class="game-share-wrap"><div class="game-share-labels">${bar.map(x=>`<button type="button" class="game-share-label" data-stat-filter="${x.id==='__others__'?'others':escape(x.id)}" style="--segment-width:${x.share}%"><strong>${Number(x.share||0).toFixed(1)}%</strong><span>${escape(x.name)}</span></button>`).join('')}</div><div class="game-share-bar" aria-label="Distribución de directos por juego" style="grid-template-columns:${bar.map(x=>`${Number(x.share)||0}fr`).join(' ')}">${top.map((x,i)=>segment(x,i)).join('')}${othersSegment}</div><div class="game-share-legend">${bar.map((x,i)=>`<button type="button" class="game-share-legend-item" data-stat-filter="${x.id==='__others__'?'others':escape(x.id)}"><i class="game-share-dot ${x.id==='__others__'?'game-share-others':'game-share-'+(i+1)}"></i><span>${escape(x.name)}</span><strong>${x.count}</strong></button>`).join('')}</div></div><div class="game-stats-controls"><input id="gameStatsSearch" type="search" placeholder="Buscar juego..." autocomplete="off"><button type="button" class="stats-filter-clear" data-stat-filter="">Todos</button><button type="button" class="stats-filter-clear" data-stat-filter="others">Solo «Otros»</button></div><div class="game-stats-table" id="gameStatsTable"></div>`;

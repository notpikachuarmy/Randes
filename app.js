...
function renderGameStats(){
 ...
 try{
   ...
   const segment=(x,i)=>{const img=gameImage(game(x.id));const share=Number(x.share)||0;const count=Number(x.count)||0;const bg=img?`background-image:url(&quot;images/${escape(img)}&quot;);`:'';return `<button type="button" class="game-share-segment game-share-${i+1}" data-stat-filter="${escape(x.id)}" style="--segment-width:${share}%;${bg}" title="${escape(x.name)} · ${count} directos · ${share.toFixed(1)}%"><span class="game-share-overlay" aria-hidden="true"></span></button>`};
